 ...
 ********************************************************************/
extern "C" magma_int_t magma_dgetrf(magma_int_t m, magma_int_t n, double *A,
	...
	*info = 0;
	...
	if ((nb <= 1) || (nb >= min(m, n))) {
		...
		if (maxdim * maxdim < 2 * maxm * maxn) {
			...
			if (MAGMA_SUCCESS
					!= magma_dmalloc(&dA, nb * maxm + maxdim * maxdim)) {
...
//        
		...
		float download_copy_time_cuda_temp, upload_copy_time_cuda_temp,
				total_copy_time_cuda = 0;
		...
		static double gpu_time_iter0_highest_freq = 0.401762,
				gpu_time_iter0_lowest_freq = 1.753552;
		...
		for (j = 0; j < s; j++) {
			...
			if (j > 0) {
				...
				magma_dtrsm(MagmaRight, MagmaUpper, MagmaNoTrans, MagmaUnit,
						n - (j + 1) * nb, nb, c_one, dAT(j-1,j-1), ldda,
						dAT(j-1,j+1), ldda);
//											 
//				if(TIME_MEASUREMENT || ALGORITHMIC_SLACK_PREDICTION)
//				{
...
//				}
				...
				magma_dgemm(MagmaNoTrans, MagmaNoTrans, n - (j + 1) * nb,
						m - j * nb, nb, c_neg_one, dAT(j-1,j+1), ldda,
						dAT(j, j-1), ldda, c_one, dAT(j, j+1), ldda);
...
//                    }
				...
				if (j == 1) {
					...
					gpu_time_pred = gpu_time_pred * ratio_slack_pred
							* ratio_slack_pred;
...
//					printf("iter %d: gpu_time_pred = %f\n", j, gpu_time_pred);
				...
				if (j > 1 && j < s * 0.25) {
					ratio_split_freq =
							(cpu_time_pred - gpu_time_pred)
									/ (gpu_time_pred
											* ((gpu_time_iter0_lowest_freq
													/ gpu_time_iter0_highest_freq)
													- 1));
					//printf("iter %d: ratio_split_freq = %f\n", j, ratio_split_freq);
					gpu_time_this_iter_lowest_freq =
							gpu_time_this_iter_lowest_freq * ratio_slack_pred
									* ratio_slack_pred;
					...
					seconds_until_interrupt = gpu_time_this_iter_lowest_freq
							* ratio_split_freq;
...
//            }
			...
			if (s > (j + 1)) {
				magma_dtrsm(MagmaRight, MagmaUpper, MagmaNoTrans, MagmaUnit, nb,
						nb, c_one, dAT(j, j ), ldda, dAT(j, j+1), ldda);
				magma_dgemm(MagmaNoTrans, MagmaNoTrans, nb, m - (j + 1) * nb,
						nb, c_neg_one, dAT(j, j+1), ldda, dAT(j+1, j ), ldda,
						c_one, dAT(j+1, j+1), ldda);
			} else {
				magma_dtrsm(MagmaRight, MagmaUpper, MagmaNoTrans, MagmaUnit,
						n - s * nb, nb, c_one, dAT(j, j ), ldda, dAT(j, j+1),
						ldda);
				magma_dgemm(MagmaNoTrans, MagmaNoTrans, n - (j + 1) * nb,
						m - (j + 1) * nb, nb, c_neg_one, dAT(j, j+1), ldda,
						dAT(j+1, j ), ldda, c_one, dAT(j+1, j+1), ldda);
			}
...
//        }
		...
		if (nb0 > 0) {
			...
			magma_dtrsm(MagmaRight, MagmaUpper, MagmaNoTrans, MagmaUnit,
					n - s * nb - nb0, nb0, c_one, dAT(s,s), ldda,
					dAT(s,s) + nb0, ldda);
		}
